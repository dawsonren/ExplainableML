...
def _ale_1d(f, X, feature_idx, bins, categorical=False):
    ...
    for k in range(K):
        if N_k[k] > 0:
            average_deltas[k] = (1 / N_k[k]) * np.sum(deltas[k_x == k])
...
def _ale_2d(
    ...
    for k in range(K):
        for m in range(M):
            mask = (k_x == k) & (m_x == m)
            if N_km[k, m] > 0:
                average_deltas[k, m] = (1 / N_km[k, m]) * np.sum(deltas[mask])
            else:
                # replace with nearest non-empty bin average
                # TODO: this is a hack, improve later, R uses ann package
                nearest_k = np.argmin(np.abs(np.arange(K) - k))
                nearest_m = np.argmin(np.abs(np.arange(M) - m))
                average_deltas[k, m] = (
                    average_deltas[nearest_k, m] + average_deltas[k, nearest_m]
                ) / 2
    # accumulate
    ...
    # cancel main effects: marginal average of delta over the other feature
    main_effect_1 = np.zeros(K)
    for k in range(K):
        if N_k[k] > 0:
            main_effect_1[k] = np.dot(average_deltas[k, :], N_km[k, :]) / N_k[k]
    main_accumulated_1 = np.pad(main_effect_1.cumsum(), (1, 0), mode="constant")

    main_effect_2 = np.zeros(M)
    for m in range(M):
        if N_m[m] > 0:
            main_effect_2[m] = np.dot(average_deltas[:, m], N_km[:, m]) / N_m[m]
